...
if($Mlist==0){
    switch ($Cat){
        case 1;
            $UserList = dbSelect('user','uid,username,picture, birthday','','birthday desc');
            break;
        case 2;
            $UserList = dbSelect('user','uid,username,picture, profession','','profession desc');
            break;
        case 3;
            $UserList = dbSelect('user','uid,username,picture, region','','region desc');
            break;
        default:
            $UserList = dbSelect('user','uid,username,picture','','username asc');
    ...
    switch ($Cat){
        case 1;
            ...
            $UserList = DBduoSelect('user as u','friend as f','on u.uid = f.fid and f.approved=1',null,null,$select,'f.uid ='.$_COOKIE['uid'].'','u.birthday desc');
            break;
        case 2;
            $select='u.uid as uid, u.username as username, u.picture as picture, u.profession as profession';
            $UserList = DBduoSelect('user as u','friend as f','on u.uid = f.fid and f.approved=1',null,null,$select,'f.uid ='.$_COOKIE['uid'].'','u.profession desc');
            break;
        case 3;
            $select='u.uid as uid, u.username as username, u.picture as picture, u.region as region';
            $UserList = DBduoSelect('user as u','friend as f','on u.uid = f.fid and f.approved=1',null,null,$select,'f.uid ='.$_COOKIE['uid'].'','u.region desc');
            break;
        default:
