 ...
 */
	...
	if($_POST['replysubmit'])
	{
		...
		$tid = $Id;					//跟帖时记录贴子ID
        $titleTmp = dbSelect('details','title','id="'.$tid.'"','id desc',1);
        ...
        $n='first, tid, authorid, title, content, addtime, addip, classid';
        $v='0, '.$tid.', '.$authorid.',"'.$title.'", "'.$content.'", '.$addtime.', '.$addip.', '.$classId.'';
